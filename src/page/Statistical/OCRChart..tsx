...
import { getStatisticalOcrHistory, getStatisticalOcrHistoryTranslate } from '~/service/task.ts';
...
const OCRChart = () => {
  ...
  const fetchData = async (groupvalue = 0, from_date?: Date, to_date?: Date) => {
    try {
      const requestParams = {
        group: groupvalue,
      ...
      const valueList = response.data.map((item: any) => dataType === 'text' ? item.ocr_converted : item.ocr_translated
      );
  ...
  useEffect(() => {
    const rangeStart = timeScale === 'day' ? oneWeekAgo() : undefined;
    const rangeEnd = timeScale === 'day'? today : undefined;
    fetchData(timeScale === 'day' ? 0 : timeScale === 'month' ? 1 : 2, startDate ?  startDate : rangeStart, endDate ? endDate : rangeEnd);
  }, [dateRange]);
    ...
    datasets: [
      {
        ...
        data: y,
        backgroundColor: dataType === 'text' ? 'rgba(75, 192, 192, 0.5)' : 'rgba(54, 162, 235, 0.5)',
        borderColor: dataType === 'text' ? 'rgba(75, 192, 192, 1)' : 'rgba(54, 162, 235, 1)',
        borderWidth: 1,
      },
    ],
  };
  ...
  return (
    <div className="w-full max-w-4xl mx-auto mt-4">
      <h2 className="text-xl font-semibold mb-4">Biểu đồ sử dụng quét và dịch ảnh</h2>
